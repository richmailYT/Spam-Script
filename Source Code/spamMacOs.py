...
def RedrawAll():
    global frame, SettingsTextTk, SettingsTextStr, files, filename#, Tempfilename
    for child in frame.winfo_children():
        child.destroy()
...
def AddFile():	
    global ExsitingFileLabel, FileLabel, filename , files #telling python the edit the gobal variable, not makeing a new one, it can also be used to make a local var and globla one
    filename = tk.filedialog.askopenfilename(initialdir='/', title='Select File', filetypes=(("text file", "*.txt"), ('all files', '*.*')))
    Tempfilename = tk.StringVar()
    ...
    FileLabel = tk.Label(frame, textvariable=Tempfilename, padx=10, pady=5, bg=Modes[mode.get()][3]['background'], fg=Modes[mode.get()][2]['text'])
    FileLabel.pack()
    ExsitingFileLabel = True
...
def StartTheSpam():
    ...
    for i in range(1, countdown_value):
...
def MakeChildren():
    global Credits, SettingsText, Settings, filename,FileLabel# mode, Modes, SettingsTextTk
    ...
    if filename != '':
        FileLabel = tk.Label(frame, textvariable=filename, padx=10, pady=5, bg=Modes[mode.get()][3]['background'], fg=Modes[mode.get()][2]['text'])
        FileLabel.pack()
